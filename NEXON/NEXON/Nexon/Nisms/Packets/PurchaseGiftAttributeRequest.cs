...
{
	public class PurchaseGiftAttributeRequest
	{
		...
		public byte ReceiverServerNo { get; set; }
		...
		public ICollection<PurchaseGiftAttributeRequest.Product> ProductArray { get; set; }

		private int CalculateStructureSize()
		{
			int num = this.RemoteIP.CalculateStructureSize() + ((byte)this.Reason).CalculateStructureSize() + this.GameID.CalculateStructureSize() + this.UserID.CalculateStructureSize() + this.UserOID.CalculateStructureSize() + this.UserName.CalculateStructureSize() + this.UserAge.CalculateStructureSize() + this.ReceiverServerNo.CalculateStructureSize() + this.ReceiverGameID.CalculateStructureSize() + this.Message.CalculateStructureSize() + this.OrderID.CalculateStructureSize() + ((int)this.PaymentType).CalculateStructureSize() + this.TotalAmount.CalculateStructureSize() + 0.CalculateStructureSize();
			int num2;
		...
		{
			Packet result = new Packet(this.CalculateStructureSize(), PacketType.PurchaseGiftAttribute);
			...
			result.Write(this.ReceiverServerNo);
			result.Write(this.ReceiverGameID);
			...
			if (this.ProductArray != null)
			{
				foreach (PurchaseGiftAttributeRequest.Product product in this.ProductArray)
				{
