...
{
	public class WriteFreeQueue2<T>
	{
		private static KeyValuePair<int, T> AttachPriority(T value)
		{
			return new KeyValuePair<int, T>(WriteFreeQueue2<T>.currentPriority, value);
		}

		public WriteFreeQueue2()
		{
			this.readCacheLength = 32;
			this.threadLocalQueue = new SingleFreeQueue2<KeyValuePair<int, T>>[32];
			this.queueIndices = new int[32];
			this.queueLength = 0;
			this.nextIndex = -1;
		}

		public WriteFreeQueue2(int readCacheLength) : this()
		{
		...
		{
			this.threadLocalQueue[threadID] = new SingleFreeQueue2<KeyValuePair<int, T>>(this.readCacheLength);
        ...
        private SingleFreeQueue2<KeyValuePair<int, T>> GetSingleFreeQueue()
        {
		...
		{
			WriteFreeQueue2<T>.currentPriority = ++this.priority;
			this.GetSingleFreeQueue().Enqueue<T>(collection, WriteFreeQueue2<T>.priorityAttacher);
		}
		...
		private static Converter<T, KeyValuePair<int, T>> priorityAttacher = new Converter<T, KeyValuePair<int, T>>(WriteFreeQueue2<T>.AttachPriority);

		private SingleFreeQueue2<KeyValuePair<int, T>>[] threadLocalQueue;
